...
@app.before_request
def before_request():
    """Make sure we are connected to the database each request and
    Look up the current user so that we know he's there."""
    g.db = connect_db()
    g.user = None
    if 'user_id' in session:  # 세션은 페이지
        g.user = query_db('select * from user where user_id = ?',
                          [session['user_id']], one=True)
        # user_id로 검색하되 하나만 들고 와라


@app.teardown_request
def teardown_request(exception):
    """Closes the database again at the end of the request."""
    if hasattr(g, 'db'):  # g : 글로벌 변수 / 안에 db 정보가 있음
        g.db.close()  # db 종료


def query_db(query, args=(), one=False):
    """Queries the database and returns a list of dictionaries."""
    cur = g.db.execute(query, args)  # 커서 변수
...
def add_message():
    ...
    if 'user_id' not in session:
        abort(401)  # 권한 없는 요청
    if request.form['text']:  # 뭔가 쓴 것이 있으면
...
def public_timeline():
    ...
    return render_template('timeline.html', messages=query_db('''
        ...
        order by message.pub_date desc limit ?''', [PER_PAGE]))  # 30개


@app.route('/')
def timeline():
    """Shows a user's timeline or if no user is logged in it
    will redirect to the public timeline.    This timeline shows
...
def user_timeline(username):
    ...
    if g.user:
        ...
        return render_template('timeline.html', messages=query_db('''
            select messages.*, user.* from message, user
